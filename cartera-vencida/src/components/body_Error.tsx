...
const Body_Error: React.FC<{ isProtected?: boolean }> = ({ isProtected }) => {
    ...
    if (!isProtected && location.pathname !== '/') {
        return (
            <Box sx={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', height: '100vh', backgroundColor: colors.background_WhiteSmoke, textAlign: 'center' }} >
                ...
                <Button variant="contained" sx={{ bgcolor: colors.oliveGreen, color: '#FFFFFF', '&:hover': { bgcolor: colors.oliveGreen } }} onClick={() => (window.location.href = '/')} >
                    Ir al inicio
                </Button>
            </Box>
        );
