...
const formatValue = (value: string) => value ? value.replace(/^NAVE\s*/, '') : '';
      ...
      <View style={styles.section05}>
        <View style={styles.roundedSquare}>
          <Text style={styles.texts} hyphenationCallback={hyphenationCallback}>
            ...
            En la ciudad de Ambato, a __________ del mes __________ año ______ siendo las _____ h _____ dentro de las instalaciones del Mercado Mayorista Ambato,encontrandome en la Nave ________ sección ________ puesto/bodega/local/núcleo/cubículo Nro. _____ adjudicado al arrendatario ___________________________________ con CC/RUC No. _______________, quien encotrándose presente y en persoa en su espacio arrendado, se niega a recibir la notificación.{"\n"}{"\n"}
...
export const generarPDF = async (selectedRow: any, notificaciones: number) => {
  ...
  const adjustedRow = {
    ...
    puesto: formatValue(selectedRow.puesto),
    bodega: formatValue(selectedRow.bodega),
    nave: formatValue(selectedRow.nave)
  }
